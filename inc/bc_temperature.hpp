...
void boundary_temperature()
{
   ...
   for (i=0; i<=(NX+1); i++)
   {
      tn(i,NY+1) = tn(i,NY); // top
      tn(i,0   ) = tn(i, 1); // bottom
// tn(i,NY+1)=2*300.16f-tn(i,NY);
   }
   for (j=0; j<=(NY+1); j++)
   {
//      tn(0    ,j) =  1.0f; // left
//      tn(NX+1 ,j) =  0.0f; // right

      tn(0    ,j) =  2*320.0-tn(1,j); //296.0+(tn(1,j)- 296.0)/2.0f;    //296.0; //-tn( 1,j)+2.0f; // left
//      tn(NX+1 ,j) =  2*290.0-tn(NX,j); //296.0+(tn(1,j)- 296.0)/2.0f;    //296.0; //-tn( 1,j)+2.0f; // left
//      tn(NX+1 ,j) =  2*290.0-tn(NX,j); //290.0+(tn(NX,j)-290.0)/2.0f; //290.0; //-tn(NX,j); // right
